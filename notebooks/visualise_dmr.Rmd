...
load("../genome_data/CpG_coordinates_mm10.RData")
bisulfite_allelic_df <- read_tsv('../bisulfite/CpG_context.combined_replicates.genome1.summary.tsv', col_names = c('chr', 'start', 'pctmeth', 'mC', 'C')) %>%
  mutate(allele="m") %>% 
  bind_rows(read_tsv('../bisulfite/CpG_context.combined_replicates.genome2.summary.tsv', col_names = c('chr', 'start', 'pctmeth', 'mC', 'C')) %>%
              mutate(allele='p')) %>%
  mutate(end=start,
         chr=sub("chr", "", chr))
ah <- AnnotationHub()
...
```{r}
load("../RData/rna_seq_with_reverse2.RData")
```
...
for (.name in unique(combined_dmr$name)) {
  try({
    ...
    p <- dmr1 %>%
    {plot_track(.$chr[1], .$start[1], .$end[1], flank=5000, rrbs=T, icr=T, reverse=T, rna=T)}
    ggbio::ggsave(paste0("../plots/dss_paired/", .name, ".png"), plot=p, width=8, height=12)
    p <- dmr1 %>%
    {plot_track(.$chr[1], .$start[1], .$end[1], flank=20000, rrbs=T, icr=T, reverse=T, rna=T)}
    ggbio::ggsave(paste0("../plots/dss_paired_20kb/", .name, ".png"), plot=p, width=8, height=12)
  })
}
```
