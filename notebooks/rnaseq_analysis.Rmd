...
#get counts
GTF <- 'Mus_musculus.GRCm38.90.gtf'
#convert to nice gene IDs with biomaRt
mouse <- useMart("ensembl", dataset = "mmusculus_gene_ensembl")

gene <- getBM(attributes = c('mgi_symbol','chromosome_name', 'strand', 'start_position', 'end_position'), mart = mouse, values = rownames(totcounts$counts))

splitfeaturecounts <- featureCounts(file.path("..", "rna_seq", splitbam), annot.ext= GTF,isGTFAnnotationFile = TRUE, useMetaFeatures = TRUE, primary = TRUE, isPairedEnd = TRUE, nthreads = 8)
...
```{r}
counts <- select(splitcounts, -GeneID)
...
embryo <- colnames(splitcounts) %>%
  str_replace("_(mat|pat)", "") %>%
  factor()
strain <- ifelse((startsWith(colnames(splitcounts), "B6") & endsWith(colnames(splitcounts), "mat")) | 
                   (startsWith(colnames(splitcounts), "Cast") & endsWith(colnames(splitcounts), "pat")), 
                 "Black6", "Cast") %>%
  factor()
litter <- case_when(str_detect(colnames(splitcounts), "^B6CastF1_(2|3)") ~ "A",
                    str_detect(colnames(splitcounts), "^B6CastF1_(4|5)") ~ "B",
                    str_detect(colnames(splitcounts), "^CastB6F1_(2|3)") ~ "C",
                    str_detect(colnames(splitcounts), "^CastB6F1_(4|5)") ~ "D") %>%
  factor()
parent <- ifelse(endsWith(colnames(splitcounts), "mat"), "maternal", "paternal") %>%
...
glMDSPlot(splitDE, labels = rownames(splitDE$samples), groups = colData)
...
annotated_imprinted <- impr10 %>% 
  ...
  select(-known_ig, -final_conclusion_on_imprinted_status)
...
glMDPlot(qlfstrain, status=dt.edger.strain, counts=counts, groups= colData$embryo, transform=TRUE, html = 'MDPlot_strain', anno = annot, display.columns=c("ID", "mgi_symbol"), sample.cols = rep(cbbPalette[1:2], 8), side.xlab = 'sample', side.ylab = 'Expression')
...
strain05log2 <- strain_de_results %>% filter(padj < 0.05 & abs(logFC) > 1)
...
```{r} 
newgenes <- filter(annotated_imprinted, in_db + in_andergassen + in_Finn + in_Calabrese + in_Inoue ==0)
```
